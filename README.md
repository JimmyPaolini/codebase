...
- **🔭 callidescope** - Call stack tracing toolchain that follows control flow through injected dependencies and reports where a stack got too deep
  ...
  - **[callidescope-nx](packages/callidescope-nx)** - Nx plugin inferring a per-project `callidescope-trace` target that follows the Nx dependency graph, keeping every Nx dependency out of the packages that trace
