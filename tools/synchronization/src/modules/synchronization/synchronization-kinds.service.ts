...
export class SynchronizationKindsService {
  ...
  private describeAcceptedKinds(problem: string): string {
    return `${problem}. It takes a comma-separated set drawn from ${SYNCHRONIZATION_KINDS.map((kind) => `"${kind}"`).join(" and ")}, as in "--kinds ${SYNCHRONIZATION_KINDS.join(SYNCHRONIZATION_KIND_SEPARATOR)}".`;
  }
