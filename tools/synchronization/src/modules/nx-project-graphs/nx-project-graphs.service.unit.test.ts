...
function readNeighborhood(
  neighborhoods: Map<string, NxProjectGraphNeighborhood>,
  projectName: string,
): NxProjectGraphNeighborhood {
...
describe(NxProjectGraphsService, () => {
  let service: NxProjectGraphsService;

  beforeAll(async () => {
    const module = await Test.createTestingModule({
      providers: [NxProjectGraphsService],
    }).compile();

    service = await module.resolve(NxProjectGraphsService);
  });
  ...
  describe("readProjectGraph", () => {
    ...
    // whole synchronization run with it.
  ...
  describe("buildNeighborhoods", () => {
    it("reports what a project depends on and what depends on it", () => {
      const graph = buildGraph({
        ...
        synchronization: [["logger", "static"]],
      });

      const neighborhood = readNeighborhood(
        service.buildNeighborhoods(
          graph,
          buildProjects(["caelundas", "logger", "synchronization"]),
        ),
        "logger",
      );
  ...
  describe("renderMermaid", () => {
    ...
    it("draws an implicit edge dotted and explains it", () => {
      ...
      expect(diagram).toContain("_Dotted edges are implicit dependencies");
    });
