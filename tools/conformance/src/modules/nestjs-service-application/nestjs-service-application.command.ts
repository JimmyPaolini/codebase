...
} from "./nestjs-service-application.types";
import type { Tree } from "@nx/devkit";

/**
 * Generates a NestJS service application scaffold from templates.
 */
@Command({
  description: "Run the nestjs-service-application command",
  name: "nestjs-service-application",
})
@Injectable()
export class NestjsServiceApplicationCommand extends CommandRunner {
  ...
  constructor(
    ...
    super();
    this.logger.setContext(NestjsServiceApplicationCommand.name);
  }
  ...
  private readonly optionsLogLabel: string =
    "NestJS service application options";
  private readonly outputFilesLogLabel: string =
    "NestJS service application output files";
  ...
  /**
   * Resolves the type for generated service applications.
   */
  @Option({
  ...
  async run(
    ...
    const substitutions: NestjsServiceApplicationSubstitutions = {
      ...this.generatorService.buildNameSubstitutions(name),
      type,
    };

    const outputFiles =
      await this.generatorService.generateFiles<NestjsServiceApplicationSubstitutions>(
        {
