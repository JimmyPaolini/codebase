# ArchiveLogs: NestJS Command-Line Application
...
```bash
nx run archive-logs:develop        # Run CLI (tsx, watch mode)
nx run archive-logs:lint           # ESLint
nx run archive-logs:typecheck      # tsc --noEmit
nx run archive-logs:format         # oxfmt formatting
nx run archive-logs:build          # Compile for production
```
...
```bash
nx run archive-logs:test:unit          # Fast (<100ms) — pure logic, mocked DI
nx run archive-logs:test:integration   # Moderate (1-2s) — real DB/API I/O
nx run archive-logs:test:end-to-end    # Slow (30-60s) — full CLI execution
```
