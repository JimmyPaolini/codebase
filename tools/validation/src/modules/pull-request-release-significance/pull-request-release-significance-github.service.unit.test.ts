...
describe(PullRequestReleaseSignificanceGithubService, () => {
  let service: PullRequestReleaseSignificanceGithubService;
  ...
  beforeAll(async () => {
    const module = await Test.createTestingModule({
      providers: [PullRequestReleaseSignificanceGithubService],
    }).compile();

    service = await module.resolve(PullRequestReleaseSignificanceGithubService);
  });
  ...
  describe("run", () => {
    it("invokes gh with the arguments it was given", () => {
      ...
      expect(service.run(["pr", "view", "7"])).toStrictEqual({
        available: true,
        standardError: "",
        standardOutput: "{}",
        succeeded: true,
    ...
    it("reports a non-zero exit as a failed call", () => {
      ...
      completeWith({ status: 1, stderr: "no such pull request" });

      expect(service.run(["pr", "view", "7"]).succeeded).toBe(false);
    });
