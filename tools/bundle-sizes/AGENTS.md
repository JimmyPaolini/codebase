...
```bash
nx run bundle-sizes:develop        # Run CLI (tsx, watch mode)
nx run bundle-sizes:lint           # ESLint
nx run bundle-sizes:typecheck      # tsc --noEmit
nx run bundle-sizes:format         # oxfmt formatting
nx run bundle-sizes:build          # Compile for production
```
...
```bash
nx run bundle-sizes:test:unit          # Fast (<100ms) — pure logic, mocked DI
nx run bundle-sizes:test:integration   # Moderate (1-2s) — real database/API I/O
nx run bundle-sizes:test:end-to-end    # Slow (30-60s) — full CLI execution
```
