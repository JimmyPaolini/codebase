/**
 * Renders the `🎒 Bundles` section of a pull request description.
 *
 * Takes the rows `scripts/collect-bundle-sizes.ts` gathered and produces one
 ...
 * did not rebuild.
 *
 * The section is wrapped in HTML comment markers so `spliceSection` can replace
 * it on every push without disturbing the description around it.
 *
 * See `scripts/report-bundle-sizes.ts` for the command-line half.
 */
import {
  type BundleRow,
  isComparable,
...
export function renderBundlesSection(
...
/**
 * Replaces the marked section in a pull request description, or appends it when
 * the description has none, leaving everything the author wrote untouched.
 */
export function spliceSection(description: string, section: string): string {
  const before = description.split(SECTION_START)[0]?.trimEnd() ?? "";
