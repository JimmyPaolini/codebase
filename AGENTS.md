...
- **`.html` is folded into that same pass.** Nothing else claims the extension, so a block giving it `pathOnlyParser` — a stub yielding an empty `Program`, exported from `eslint.config.ts` — is all it takes. That stub must carry a `meta` name: ESLint serializes the parser into `.eslintcache` and refuses a custom one that cannot name itself.
- **A second pass** (`eslint-structure`, `configuration/eslint-structure.config.ts`) covers `.md` alone, sharing that same parser. Markdown resolves to `@eslint/markdown`'s `markdown/gfm` language, whose tree is mdast and carries no `Program`, and ESLint allows exactly one `language` per file. The two cannot be reconciled: giving `.md` the stub parser without overriding `language` leaves the markdown rules working and the structure rule silently never firing, and overriding `language` to `"@/js"` so it does fire is rejected outright with `Key "languageOptions": Unexpected key "frontmatter" found.` Folding markdown in therefore means deleting the `@eslint/markdown` block — about fifteen rules, `no-html` among them, which this repository enables precisely because `markdownlint`'s MD033 is off. The second pass reads the same `codebase-structure.json`, takes no `write` configuration (there is nothing to fix automatically, only a path to move), and joins `lint-codebase` beside `eslint`.
