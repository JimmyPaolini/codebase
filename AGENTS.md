...
- **A `src/` root holds entry points and nothing else.** A file sitting directly in `src/` is named for one: bare — `bootstrap`, `client`, `constants`, `index`, `main`, `repl`, `router`, or lexico's generated `routeTree.gen` — or one of `bootstrap`, `index`, `languages`, `main`, `plugin-context`, `repl`, `router` carrying a `constants`, `module`, or `utilities` suffix, as in `main.module.ts`. Either form may take `.unit.test`, `.integration.test`, or `.end-to-end.test` before the extension. A `src/plugin.ts` entrypoint is forbidden. Anything else — a `scratch.ts`, a stray `.json` — is a lint error: put it under `src/modules/` or outside `src/`.
