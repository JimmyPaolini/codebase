...
/**
 * How many documents `DrawCommand` commits: 357 named patterns beside 3,179
 ...
 * spot.
 */
const COMMITTED_CORPUS_SIZE = 357 + 3179;
...
describe(MeanderTopologyService, () => {
  ...
  describe("the meander charter", () => {
    ...
    //
    // The count also pins where the sweep stops. 357 is every combination up
    ...
    it("sweeps every named-type combination DrawCommand writes, out to the deepest row count the command line accepts", () => {
      expect(charterSweep).toHaveLength(357);
      expect(
    ...
    // trees and every other one of the 3,536 is not. Reading from disk is
    ...
    it("branches in exactly the families the charter names, measured from disk", async () => {
      ...
      expect(documents).toHaveLength(357);
