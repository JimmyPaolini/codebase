...
import type { EnumeratedMeander } from "./meander-enumeration.types";
 ...
 * `MosaicTilesService` already decides each of a repeat's edges in turn,
 * `2 ** edges` wide, folded by `MosaicSymmetryService`'s group so that a
 ...
 * `MeanderClassificationService` rather than by which generator drew
 * something.
 *
 * **The budget is the lattice's, not the family's.**
 * `MOSAIC_TILE_EDGE_BUDGET` is read through `MosaicTilesService` rather than
 ...
 */
@Injectable()
export class MeanderEnumerationService {
  ...
  isAdmitted(shape: MeanderShape): boolean {
    return this.mosaicTilesService.isAdmitted(shape);
  }
   ...
   * {@link MEANDER_ENUMERATION_MINIMUM_ROWS} and climbs while a
  ...
  shapes(): MeanderShape[] {
    ...
    for (
      let rows = MEANDER_ENUMERATION_MINIMUM_ROWS;
      this.isAdmitted({ columns: 1, rows });
      rows += 1
    ) {
      const widest = this.mosaicTilesService.maximumColumns(rows);
