...
describe(MLetterCountCharacteristicService, () => {
  let contextService: CharacteristicContextService;
  let service: MLetterCountCharacteristicService;
  ...
  beforeAll(async () => {
    ...
    service = await module.resolve(MLetterCountCharacteristicService);
  });
  ...
  it("counts an isolated M glyph, legs pointing south", () => {
  ...
  it("ignores the glyph when more ink joins it", () => {
    expect(service.compute(contextService.create("04x03y6771c8c08080"))).toBe(
      0,
    );
  });
