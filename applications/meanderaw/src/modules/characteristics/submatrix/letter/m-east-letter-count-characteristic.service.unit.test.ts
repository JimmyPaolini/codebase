...
describe(MEastLetterCountCharacteristicService, () => {
  let contextService: CharacteristicContextService;
  let service: MEastLetterCountCharacteristicService;
  ...
  beforeAll(async () => {
    ...
    service = await module.resolve(MEastLetterCountCharacteristicService);
  });
  ...
  it("counts an isolated M glyph, legs pointing east", () => {
  ...
  it("ignores the glyph when more ink joins it", () => {
    expect(service.compute(contextService.create("04x03y6331e100a310"))).toBe(
      0,
    );
  });
