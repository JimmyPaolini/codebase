 ...
 * it. `CodeService.spellCanonical` says which tile this is; this says
 * what kind of tile it is, where there is a kind to say.
 *
 ...
 *   split on {@link CornerLanes} instead, whose two halves are false
...
export class SubFamilyService {
   ...
   */
  private cornerLanes(tile: Tile): CornerLanes {
    const { horizontal } = this.tileService.edges(tile);
  ...
  private everyPoint(
    tile: Tile,
    predicate: (directions: Directions) => boolean,
  ): boolean {
  ...
  private isCorner(directions: Directions): boolean {
    return (
      this.tileService.degree(directions) === 2 &&
  ...
  private isHorizontal(directions: Directions): boolean {
    return (
      (directions.east || directions.west) &&
      !directions.north &&
      !directions.south
   ...
   */
  private isUnbroken(tile: Tile): UnbrokenRuns {
  ...
  private isVertical(directions: Directions): boolean {
    return (
      (directions.north || directions.south) &&
      !directions.east &&
      !directions.west
   ...
   * `sub-family.service.unit.test.ts` asserts against the whole
   * enumerated space.
   */
  matching(tile: Tile): SubFamily[] {
    return this.rules()
  ...
  name(tile: Tile): SubFamily | undefined {
   ...
   * are like this.
   */
  rules(): readonly SubFamilyRule[] {
    const bare = (tile: Tile): boolean =>
      this.everyPoint(tile, (point) => this.tileService.isBare(point));
    const corner = (tile: Tile): boolean =>
      this.everyPoint(tile, (point) => this.isCorner(point));
    const horizontal = (tile: Tile): boolean =>
      this.everyPoint(tile, (point) => this.isHorizontal(point));
    const vertical = (tile: Tile): boolean =>
