...
```bash
pnpm exec nx run conformetry-cli:start -- generate --generator case-variants --name 'search bar' --nameCamelCase spelledOutByHand --owner platform --no-interactive --directory tmp/conformetry-examples/case-variants --config packages/conformetry-examples/examples/case-variants/conformetry.config.ts
```
