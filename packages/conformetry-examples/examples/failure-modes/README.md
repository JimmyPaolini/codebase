...
```bash
pnpm exec nx run conformetry-cli:start -- generate --template missing-input --name unowned --directory tmp/conformetry-examples/failure-modes --config packages/conformetry-examples/examples/failure-modes/conformetry.config.ts
```
