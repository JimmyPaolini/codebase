# CodometerCustomStatistics: NestJS Service Application
...
```bash
nx run codometer-custom-statistics:lint-codebase   # Every static check, in one graph
nx run codometer-custom-statistics:typecheck       # tsc --noEmit
nx run codometer-custom-statistics:oxfmt           # Formatting
nx run codometer-custom-statistics:build           # Compile for publication
```
...
```bash
nx run codometer-custom-statistics:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codometer-custom-statistics:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codometer-custom-statistics:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
