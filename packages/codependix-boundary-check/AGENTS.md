# CodependixBoundaryCheck: NestJS Service Application
...
```bash
nx run codependix-boundary-check:lint-codebase   # Every static check, in one graph
nx run codependix-boundary-check:typecheck       # tsc --noEmit
nx run codependix-boundary-check:oxfmt           # Formatting
nx run codependix-boundary-check:build           # Compile for publication
```
...
```bash
nx run codependix-boundary-check:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-boundary-check:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-boundary-check:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
