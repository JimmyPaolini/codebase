...
```bash
nx run codependix-imports:lint-codebase   # Every static check, in one graph
nx run codependix-imports:typecheck       # tsc --noEmit
nx run codependix-imports:oxfmt           # Formatting
nx run codependix-imports:build           # Compile for publication
```
...
```bash
nx run codependix-imports:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-imports:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-imports:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
