...
nx run conformetry-jupyter:develop
```
...
```bash
nx run conformetry-jupyter:develop        # Run service (tsx, watch mode)
nx run conformetry-jupyter:lint           # ESLint
nx run conformetry-jupyter:typecheck      # tsc --noEmit
nx run conformetry-jupyter:format         # oxfmt formatting
nx run conformetry-jupyter:build          # Compile for production
```
...
```bash
nx run conformetry-jupyter:test:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-jupyter:test:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-jupyter:test:end-to-end    # Slow (30-60s) — full service initialization
```
