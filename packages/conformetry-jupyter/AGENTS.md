# ConformetryJupyter: NestJS Service Application
...
```bash
nx run conformetry-jupyter:lint-codebase   # Every static check, in one graph
nx run conformetry-jupyter:typecheck       # tsc --noEmit
nx run conformetry-jupyter:oxfmt           # Formatting
nx run conformetry-jupyter:build           # Compile for publication
```
...
```bash
nx run conformetry-jupyter:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-jupyter:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-jupyter:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
