...
import depthExecutor from "./executor";
...
describe(depthExecutor, () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("delegates to the shared runner, asking for the depth lookup", async () => {
    expect.hasAssertions();

    const context = buildContext();
    const options = { addresses: ["src/foo.service.ts#FooService.bar"] };

    await expect(depthExecutor(options, context)).resolves.toStrictEqual({
      success: true,
    });
