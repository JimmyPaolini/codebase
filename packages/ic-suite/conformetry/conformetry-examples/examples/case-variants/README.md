...
```bash
pnpm exec nx run conformetry-cli:start -- generate --template case-variants --name 'search bar' --nameCamelCase spelledOutByHand --owner platform --directory tmp/conformetry-examples/case-variants --config packages/ic-suite/conformetry/conformetry-examples/examples/case-variants/conformetry.config.ts
```
