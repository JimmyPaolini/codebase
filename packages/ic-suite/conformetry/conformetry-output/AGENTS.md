# ConformetryOutput: NestJS Service Application
...
```bash
nx run conformetry-output:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
nx run conformetry-output:typecheck       # tsc --noEmit
nx run conformetry-output:oxfmt           # Formatting
nx run conformetry-output:build           # Compile for publication
```
...
```bash
nx run conformetry-output:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-output:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-output:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
