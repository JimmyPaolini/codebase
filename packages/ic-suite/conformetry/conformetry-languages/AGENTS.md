# ConformetryLanguages: NestJS Service Application
...
```bash
nx run conformetry-languages:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
nx run conformetry-languages:typecheck       # tsc --noEmit
nx run conformetry-languages:oxfmt           # Formatting
nx run conformetry-languages:build           # Compile for publication
```
...
```bash
nx run conformetry-languages:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-languages:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-languages:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
