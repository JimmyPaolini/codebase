...
Call stacks traced through `packages/ic-suite/callidescope/callidescope-configuration`, deepest first. Each frame shows what it takes, what it returns, and what its documentation says.
...
```text
🚀 flatMap(…)(…): string[] [packages/ic-suite/callidescope/callidescope-configuration/src/modules/configuration/configuration.constants.ts:227]
  └─> readPermittedNames(…): string[] [packages/ic-suite/callidescope/callidescope-configuration/src/modules/configuration/configuration.constants.ts:146]
     ↳ Reads the names one classified field contributes to the permitted set.
    └─> map(…)([member]: [string, ProjectFieldPermission]): string [packages/ic-suite/callidescope/callidescope-configuration/src/modules/configuration/configuration.constants.ts:156]
```
