...
default six — is gone because `packages/ic-suite/codometer/codometer-configuration` declares eight,
...
```bash
nx run callidescope-examples:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase --configuration=write   # first — see below
nx run callidescope-examples:examples:write                        # regenerate output/ and the three published sections
