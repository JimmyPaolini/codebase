# CallidescopeCore: NestJS Service Application
...
```bash
nx run callidescope-core:typecheck-code,lint-code,format-code,deprecate-code,guard-code   # Every static check, in one graph
nx run callidescope-core:typecheck       # tsc --noEmit
nx run callidescope-core:oxfmt           # Formatting
nx run callidescope-core:build           # Compile for publication
```
...
```bash
nx run callidescope-core:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run callidescope-core:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run callidescope-core:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
