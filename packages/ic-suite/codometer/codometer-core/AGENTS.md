# CodometerCore: NestJS Service Application
...
```bash
nx run codometer-core:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
nx run codometer-core:typecheck       # tsc --noEmit
nx run codometer-core:oxfmt           # Formatting
nx run codometer-core:build           # Compile for publication
```
...
```bash
nx run codometer-core:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codometer-core:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codometer-core:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
