# CodometerMeasurement: NestJS Service Application
...
```bash
nx run codometer-measurement:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
...
```bash
nx run codometer-measurement:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codometer-measurement:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codometer-measurement:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
