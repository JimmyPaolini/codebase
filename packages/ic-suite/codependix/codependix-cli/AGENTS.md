# CodependixCli: NestJS Command-Line Application
...
nx run codependix-cli:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
...
```bash
nx run codependix-cli:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-cli:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-cli:vitest:end-to-end    # Slow (30-60s) — full CLI execution
```
