# CodependixCore: NestJS Service Application
...
```bash
nx run codependix-core:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
nx run codependix-core:typecheck       # tsc --noEmit
nx run codependix-core:oxfmt           # Formatting
nx run codependix-core:build           # Compile for publication
```
...
```bash
nx run codependix-core:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-core:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-core:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
