# CodependixBoundaries: NestJS Service Application
...
```bash
nx run codependix-boundaries:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
...
```bash
nx run codependix-boundaries:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-boundaries:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-boundaries:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
