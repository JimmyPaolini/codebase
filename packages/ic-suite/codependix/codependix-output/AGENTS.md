# CodependixOutput: NestJS Service Application
...
```bash
nx run codependix-output:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
nx run codependix-output:typecheck       # tsc --noEmit
nx run codependix-output:oxfmt           # Formatting
nx run codependix-output:build           # Compile for publication
```
...
```bash
nx run codependix-output:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-output:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-output:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
