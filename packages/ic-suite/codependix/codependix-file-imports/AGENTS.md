...
```bash
nx run codependix-file-imports:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
nx run codependix-file-imports:typecheck       # tsc --noEmit
nx run codependix-file-imports:oxfmt           # Formatting
nx run codependix-file-imports:build           # Compile for publication
```
...
```bash
nx run codependix-file-imports:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-file-imports:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-file-imports:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
