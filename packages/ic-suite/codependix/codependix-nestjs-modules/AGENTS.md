...
```bash
nx run codependix-nestjs-modules:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
nx run codependix-nestjs-modules:typecheck       # tsc --noEmit
nx run codependix-nestjs-modules:oxfmt           # Formatting
nx run codependix-nestjs-modules:build           # Compile for publication
```
...
```bash
nx run codependix-nestjs-modules:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-nestjs-modules:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-nestjs-modules:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
