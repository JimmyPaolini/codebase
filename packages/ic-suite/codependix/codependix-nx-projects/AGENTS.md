...
```bash
nx run codependix-nx-projects:type-codebase,lint-codebase,form-codebase,scrub-codebase,gate-codebase   # Every static check, in one graph
nx run codependix-nx-projects:typecheck       # tsc --noEmit
nx run codependix-nx-projects:oxfmt           # Formatting
nx run codependix-nx-projects:build           # Compile for publication
```
...
```bash
nx run codependix-nx-projects:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-nx-projects:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-nx-projects:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
