# CodependixConfiguration: NestJS Service Application
...
```bash
nx run codependix-configuration:type-codebase,lint-codebase,tidy-codebase,form-codebase,gate-codebase   # Every static check, in one graph
...
```bash
nx run codependix-configuration:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-configuration:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-configuration:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
