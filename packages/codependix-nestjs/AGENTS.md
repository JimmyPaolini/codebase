...
```bash
nx run codependix-nestjs:lint-codebase   # Every static check, in one graph
nx run codependix-nestjs:typecheck       # tsc --noEmit
nx run codependix-nestjs:oxfmt           # Formatting
nx run codependix-nestjs:build           # Compile for publication
```
...
```bash
nx run codependix-nestjs:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-nestjs:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-nestjs:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
