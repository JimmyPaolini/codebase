 ...
 *
 * Split out of `MapService` purely to keep that file under this
 * repository's per-file line limit — `MapService` still owns every
 ...
 * `MapService.run` can collect it for combined output without building the
...
export class WorkspaceGraphsService {
   ...
   * own, unlike the other two whole-workspace graphs, since `MapService`
