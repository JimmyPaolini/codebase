...
are traced is settled from the workspace configuration's `exclude` and
`excludeFrom` during discovery, and project configurations are loaded only
afterwards, from the projects discovery turned out to reach. Keep a project's
noise in the workspace file.
