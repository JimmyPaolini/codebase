# ConformetryText: NestJS Service Application
...
nx run conformetry-text:develop
```
...
```bash
nx run conformetry-text:develop        # Run service (tsx, watch mode)
nx run conformetry-text:lint           # ESLint
nx run conformetry-text:typecheck      # tsc --noEmit
nx run conformetry-text:format         # oxfmt formatting
nx run conformetry-text:build          # Compile for production
```
...
```bash
nx run conformetry-text:test:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-text:test:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-text:test:end-to-end    # Slow (30-60s) — full service initialization
```
