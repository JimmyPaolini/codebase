...
```bash
nx run conformetry-text:lint-codebase   # Every static check, in one graph
nx run conformetry-text:typecheck       # tsc --noEmit
nx run conformetry-text:oxfmt           # Formatting
nx run conformetry-text:build           # Compile for publication
```
...
```bash
nx run conformetry-text:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-text:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-text:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
