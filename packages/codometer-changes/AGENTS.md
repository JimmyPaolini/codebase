# CodometerChanges: NestJS Service Application
...
```bash
nx run codometer-changes:lint-codebase   # Every static check, in one graph
nx run codometer-changes:typecheck       # tsc --noEmit
nx run codometer-changes:oxfmt           # Formatting
nx run codometer-changes:build           # Compile for publication
```
...
```bash
nx run codometer-changes:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codometer-changes:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codometer-changes:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
