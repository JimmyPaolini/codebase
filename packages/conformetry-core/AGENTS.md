...
nx run conformetry-core:develop
```
...
```bash
nx run conformetry-core:develop        # Run service (tsx, watch mode)
nx run conformetry-core:lint           # ESLint
nx run conformetry-core:typecheck      # tsc --noEmit
nx run conformetry-core:format         # oxfmt formatting
nx run conformetry-core:build          # Compile for production
```
...
```bash
nx run conformetry-core:test:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-core:test:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-core:test:end-to-end    # Slow (30-60s) — full service initialization
```
