# CodometerSizeAnalysis: NestJS Service Application
...
```bash
nx run codometer-size-analysis:lint-codebase   # Every static check, in one graph
nx run codometer-size-analysis:typecheck       # tsc --noEmit
nx run codometer-size-analysis:oxfmt           # Formatting
nx run codometer-size-analysis:build           # Compile for publication
```
...
```bash
nx run codometer-size-analysis:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codometer-size-analysis:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codometer-size-analysis:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
