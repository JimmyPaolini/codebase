...
```bash
nx run conformetry-json:lint-codebase   # Every static check, in one graph
nx run conformetry-json:typecheck       # tsc --noEmit
nx run conformetry-json:oxfmt           # Formatting
nx run conformetry-json:build           # Compile for publication
```
...
```bash
nx run conformetry-json:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-json:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-json:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
