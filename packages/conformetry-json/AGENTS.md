# ConformetryJson: NestJS Service Application
...
nx run conformetry-json:develop
```
...
```bash
nx run conformetry-json:develop        # Run service (tsx, watch mode)
nx run conformetry-json:lint           # ESLint
nx run conformetry-json:typecheck      # tsc --noEmit
nx run conformetry-json:format         # oxfmt formatting
nx run conformetry-json:build          # Compile for production
```
...
```bash
nx run conformetry-json:test:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-json:test:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-json:test:end-to-end    # Slow (30-60s) — full service initialization
```
