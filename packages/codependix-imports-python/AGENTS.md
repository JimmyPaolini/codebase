# CodependixImportsPython: NestJS Service Application
...
```bash
nx run codependix-imports-python:lint-codebase   # Every static check, in one graph
nx run codependix-imports-python:typecheck       # tsc --noEmit
nx run codependix-imports-python:oxfmt           # Formatting
nx run codependix-imports-python:build           # Compile for publication
```
...
```bash
nx run codependix-imports-python:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codependix-imports-python:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codependix-imports-python:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
