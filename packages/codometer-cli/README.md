...
A second command reports what a build weighs rather than what its source says.
`bundles` reads the `size-limit-report.json` each project's `bundlesize` target
wrote, compares it to a baseline snapshot of the same reports, and renders a
markdown section:

```bash
codometer bundles --baseline .bundle-baseline --markdown REPORT.md
```
...
| `--markdown <file>` | Splice the section into this file, between its own anchor markers |
| `--output <file>` | Write the section alone to this file |

With no destination the section goes to standard output.

The section reports each bundle's size, its baseline size, the byte and
percentage change, its limit, and how much of that limit it consumes; a
per-project subtotal; a headline total; and separate counts of bundles that
appeared or disappeared. Totals only ever compare bundles both sides measured,
so renaming one reports an addition and a removal rather than a saving.

Splicing into a file is the whole of its output contract. Getting that section
in front of a reviewer — into a pull request description, say — is the caller's
job, which keeps this tool independent of any one forge.
