# CallidescopeOutput: NestJS Service Application
...
```bash
nx run callidescope-output:lint-codebase   # Every static check, in one graph
nx run callidescope-output:typecheck       # tsc --noEmit
nx run callidescope-output:oxfmt           # Formatting
nx run callidescope-output:build           # Compile for publication
```
...
```bash
nx run callidescope-output:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run callidescope-output:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run callidescope-output:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
