 ...
 *
 * A document is data rather than prose: `ExamplesService` renders it, and
 * `examples --check` compares the rendering against what is committed. That is
...
export interface ExampleJsonExport {
   ...
   * declares for every graph codependix writes — see example 15.
   */
...
/** Command-line options the `examples` command accepts. */
