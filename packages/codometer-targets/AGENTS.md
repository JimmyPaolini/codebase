# CodometerTargets: NestJS Service Application
...
```bash
nx run codometer-targets:lint-codebase   # Every static check, in one graph
nx run codometer-targets:typecheck       # tsc --noEmit
nx run codometer-targets:oxfmt           # Formatting
nx run codometer-targets:build           # Compile for publication
```
...
```bash
nx run codometer-targets:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codometer-targets:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codometer-targets:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
