# CodometerCustomization: NestJS Service Application
...
```bash
nx run codometer-customization:lint-codebase   # Every static check, in one graph
nx run codometer-customization:typecheck       # tsc --noEmit
nx run codometer-customization:oxfmt           # Formatting
nx run codometer-customization:build           # Compile for publication
```
...
```bash
nx run codometer-customization:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codometer-customization:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codometer-customization:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
