...
```bash
nx run conformetry-markdown:lint-codebase   # Every static check, in one graph
nx run conformetry-markdown:typecheck       # tsc --noEmit
nx run conformetry-markdown:oxfmt           # Formatting
nx run conformetry-markdown:build           # Compile for publication
```
...
```bash
nx run conformetry-markdown:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-markdown:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-markdown:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
