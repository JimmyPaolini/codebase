# ConformetryMarkdown: NestJS Service Application
...
nx run conformetry-markdown:develop
```
...
```bash
nx run conformetry-markdown:develop        # Run service (tsx, watch mode)
nx run conformetry-markdown:lint           # ESLint
nx run conformetry-markdown:typecheck      # tsc --noEmit
nx run conformetry-markdown:format         # oxfmt formatting
nx run conformetry-markdown:build          # Compile for production
```
...
```bash
nx run conformetry-markdown:test:unit          # Fast (<100ms) — pure logic, mocked DI
nx run conformetry-markdown:test:integration   # Moderate (1-2s) — real database/API I/O
nx run conformetry-markdown:test:end-to-end    # Slow (30-60s) — full service initialization
```
