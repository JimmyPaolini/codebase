# CodometerFileDiscovery: NestJS Service Application
...
```bash
nx run codometer-file-discovery:lint-codebase   # Every static check, in one graph
nx run codometer-file-discovery:typecheck       # tsc --noEmit
nx run codometer-file-discovery:oxfmt           # Formatting
nx run codometer-file-discovery:build           # Compile for publication
```
...
```bash
nx run codometer-file-discovery:vitest:unit          # Fast (<100ms) — pure logic, mocked DI
nx run codometer-file-discovery:vitest:integration   # Moderate (1-2s) — real database/API I/O
nx run codometer-file-discovery:vitest:end-to-end    # Slow (30-60s) — full service initialization
```
